...
export async function POST() {
  try {
    ...
    const [agenticWF, llmWF, multimodalWF, evalWF, prefWF] = await Workflow.insertMany([
      {
        ...
        description: 'Evaluate AI agents completing multi-step tasks on live Kenyan websites using the LabelForge browser extension',
    ...
    const [batchWebEC, batchWebGov, batchSocial, batchFintech] = await Batch.insertMany([
      {
        ...
        description: 'Purchase-flow and product-discovery tasks on Jumia, Kilimall, Pigiame, Naivas Online, and Jambojet',
        instructions: '1. Open the external URL with the LabelForge extension active\n2. Complete the assigned task exactly as described\n3. Screenshot every distinct UI state encountered\n4. Note obstacles, unexpected flows, or errors in the Notes field\n5. Submit only when all required screenshots are captured',
      ...
      {
        workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
        title: 'Web Navigation — Mobile Money & Banking',
        description: 'Account access, fund transfers, loan applications, and statement downloads on KCB, Equity, NCBA, Co-operative Bank, and M-Pesa portals',
        instructions: '1. Use provided test credentials only — never use real banking details\n2. Complete the full navigation flow as described\n3. Screenshot each step including confirmation dialogs or error messages\n4. If an OTP is required, note it as an obstacle and screenshot the prompt\n5. Do NOT proceed past any transaction confirmation screen',
    ...
    const t1 = await Task.create({
      ...
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Search and add item to wishlist on Jumia Kenya',
      description: 'Navigate to Jumia Kenya, search for "Samsung Galaxy Buds", apply the price filter (under KSh 5,000), open the top result, and add it to your wishlist. Screenshot the wishlist confirmation modal.',
      taskType: 'agentic-ai', status: 'unclaimed', priority: 0.95,
      difficulty: 'easy', languageTags: ['en'],
      externalUrl: 'https://www.jumia.co.ke', estimatedDuration: 20,
    ...
    const t2 = await Task.create({
      batchId: batchWebEC._id, batchTitle: batchWebEC.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Book a matatu seat on Buupass — Nairobi to Mombasa',
      description: 'Go to Buupass.com, search for a Nairobi→Mombasa seat departing tomorrow morning, select the first available seat, fill passenger details with test data, and proceed to the payment page. Do NOT complete payment — screenshot the payment form.',
      taskType: 'agentic-ai', status: 'in-progress', priority: 0.90,
      difficulty: 'medium', languageTags: ['en', 'sw'],
      externalUrl: 'https://www.buupass.com', estimatedDuration: 30,
    ...
    const t3 = await Task.create({
      batchId: batchWebEC._id, batchTitle: batchWebEC.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Register a new M-Pesa Paybill on Safaricom Business Portal',
      description: 'Log into the Safaricom Business Portal using test credentials, navigate to Lipa Na M-Pesa → Paybill Registration, and simulate registering a new Paybill number. Screenshot each step of the registration wizard.',
      taskType: 'agentic-ai', status: 'submitted', priority: 0.88,
      difficulty: 'hard', languageTags: ['en', 'sw'],
      externalUrl: 'https://www.safaricom.co.ke/business', estimatedDuration: 45,
    ...
    const t4 = await Task.create({
      batchId: batchWebEC._id, batchTitle: batchWebEC.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Compare motor insurance quotes on Policyhouse Kenya',
      description: 'Visit Policyhouse.ke, compare at least 3 comprehensive motor insurance quotes for a 2020 Toyota Vitz (1000cc), capture the full comparison table, and note the lowest-rate provider.',
      taskType: 'agentic-ai', status: 'in-review', priority: 0.85,
      difficulty: 'medium', languageTags: ['en'],
      externalUrl: 'https://www.policyhouse.co.ke', estimatedDuration: 35,
      annotatorId: wanjiru._id, annotatorEmail: wanjiru.email,
    ...
    const t5 = await Task.create({
      batchId: batchWebEC._id, batchTitle: batchWebEC.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Download PIN certificate from KRA iTax portal',
      description: 'Log into KRA iTax using test credentials, navigate to the Certificates section, download a PIN certificate, and capture the full PDF preview. Document the exact navigation path from the login page to the download.',
      taskType: 'agentic-ai', status: 'revision-requested', priority: 0.82,
      difficulty: 'hard', languageTags: ['en'],
      externalUrl: 'https://itax.kra.go.ke', estimatedDuration: 40,
    ...
    const t6 = await Task.create({
      batchId: batchWebEC._id, batchTitle: batchWebEC.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Search and wishlist a laptop on Kilimall Kenya',
      description: 'Go to Kilimall.co.ke, search for "HP laptop", filter by price range KSh 30,000–60,000, open the top-rated result, and add it to your wishlist. Screenshot the wishlist page confirming the item was added.',
      taskType: 'agentic-ai', status: 'unclaimed', priority: 0.83,
      difficulty: 'easy', languageTags: ['en'],
      externalUrl: 'https://www.kilimall.co.ke', estimatedDuration: 18,
    ...
    const t7 = await Task.create({
      batchId: batchWebEC._id, batchTitle: batchWebEC.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Book a domestic flight on Jambojet website',
      description: 'Go to Jambojet.com, search for a Nairobi NBO → Mombasa MBA flight next week (one-way, 1 adult), select the cheapest available fare, fill in passenger details with test data, and screenshot the booking summary. Do NOT proceed to payment.',
      taskType: 'agentic-ai', status: 'data-ready', isLocked: true, priority: 0.85,
      difficulty: 'medium', languageTags: ['en'],
      externalUrl: 'https://www.jambojet.com', estimatedDuration: 25,
      annotatorId: odhiambo._id, annotatorEmail: odhiambo.email,
      reviewerId: amina._id, reviewerEmail: amina.email,
      qualityScore: 93,
    ...
    const t8 = await Task.create({
      batchId: batchWebEC._id, batchTitle: batchWebEC.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Find and apply a discount coupon on Naivas Online',
      description: 'Go to shop.naivas.online, browse the Offers section, find a product with an active discount coupon, add it to your cart, and apply the coupon at checkout. Screenshot each step including the coupon field and the discounted total.',
      taskType: 'agentic-ai', status: 'rejected', priority: 0.80,
      difficulty: 'medium', languageTags: ['en', 'sw'],
      externalUrl: 'https://shop.naivas.online', estimatedDuration: 28,
    ...
    const t9 = await Task.create({
      batchId: batchWebEC._id, batchTitle: batchWebEC.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Browse electronics and compare listings on Pigiame Kenya',
      description: 'Go to Pigiame.co.ke, navigate to Electronics → Computers & Tablets, find at least 3 laptop listings, and compare their prices and specifications side-by-side using the compare feature. Screenshot the category page, each selected listing, and the comparison view.',
      taskType: 'agentic-ai', status: 'unclaimed', priority: 0.78,
      difficulty: 'easy', languageTags: ['en'],
      externalUrl: 'https://www.pigiame.co.ke', estimatedDuration: 22,
    ...
    const t10 = await Task.create({
      batchId: batchWebEC._id, batchTitle: batchWebEC.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Get a motor insurance quote on Jubilee Insurance Kenya portal',
      description: 'Visit the Jubilee Insurance Kenya website, navigate to Personal Insurance → Motor Insurance, fill in the online quote form for a 2019 Toyota Axio (1500cc, private use), get a comprehensive cover quote, and proceed to the application form page. Screenshot each step.',
      taskType: 'agentic-ai', status: 'submitted', priority: 0.76,
      difficulty: 'medium', languageTags: ['en'],
      externalUrl: 'https://www.jubileeinsurance.com', estimatedDuration: 30,
      annotatorId: kipchoge._id, annotatorEmail: kipchoge.email,
    ...
    const t11 = await Task.create({
      ...
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Apply for Huduma Number on eCitizen portal',
      description: 'Visit eCitizen.go.ke, navigate to Huduma Centre, and begin a Huduma Number application. Fill in the personal details form using placeholder values only. Screenshot the submission confirmation page.',
      taskType: 'agentic-ai', status: 'unclaimed', priority: 0.80,
      difficulty: 'medium', languageTags: ['en', 'sw'],
      externalUrl: 'https://www.ecitizen.go.ke', estimatedDuration: 30,
    ...
    const t12 = await Task.create({
      batchId: batchWebGov._id, batchTitle: batchWebGov.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Search land parcel history on Ardhisasa',
      description: 'Log into Ardhisasa.go.ke with test credentials, search for land parcel KMB/001/234 in Kiambu County, view the title deed history, and capture the full ownership timeline page.',
      taskType: 'agentic-ai', status: 'paused', priority: 0.78,
      difficulty: 'hard', languageTags: ['en'],
      externalUrl: 'https://ardhisasa.lands.go.ke', estimatedDuration: 45,
      annotatorId: wanjiru._id, annotatorEmail: wanjiru.email,
      startedAt: ago(h(3)),
    ...
    const t13 = await Task.create({
      batchId: batchWebGov._id, batchTitle: batchWebGov.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Send money via M-Pesa Developer Simulator',
      description: 'Using the M-Pesa STK Push simulator on the Safaricom Developer Portal, initiate a KSh 500 transaction. Capture the API request payload, the simulated phone confirmation prompt, and the transaction success response with the reference ID.',
      taskType: 'agentic-ai', status: 'data-ready', isLocked: true, priority: 0.92,
      difficulty: 'easy', languageTags: ['en', 'sw'],
      externalUrl: 'https://developer.safaricom.co.ke/docs', estimatedDuration: 20,
      annotatorId: njeri._id, annotatorEmail: njeri.email,
      reviewerId: amina._id, reviewerEmail: amina.email,
      qualityScore: 96,
    ...
    const t14 = await Task.create({
      batchId: batchWebGov._id, batchTitle: batchWebGov.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Renew a business permit on Nairobi City County eCitizen',
      description: 'Visit the Nairobi City County services portal, navigate to Business Permit Renewal, and initiate a renewal using fictional small business test data. Screenshot each form step and the payment summary page. Do NOT complete payment.',
      taskType: 'agentic-ai', status: 'unclaimed', priority: 0.76,
      difficulty: 'medium', languageTags: ['en', 'sw'],
      externalUrl: 'https://nairobi.go.ke/ecitizen', estimatedDuration: 35,
    ...
    const t15 = await Task.create({
      batchId: batchWebGov._id, batchTitle: batchWebGov.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Apply for NTSA driving license replacement online',
      description: 'Go to the NTSA eCitizen portal, navigate to Driving License → Request Replacement for lost/damaged license, fill in the application form with test data, and screenshot every step through to the submission confirmation.',
      taskType: 'agentic-ai', status: 'in-progress', priority: 0.74,
      difficulty: 'hard', languageTags: ['en'],
      externalUrl: 'https://www.ntsa.go.ke', estimatedDuration: 40,
      annotatorId: odhiambo._id, annotatorEmail: odhiambo.email,
      startedAt: ago(h(1.5)),
    ...
    const t16 = await Task.create({
      batchId: batchWebGov._id, batchTitle: batchWebGov.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Pay Kenya Power electricity bill via the self-service portal',
      description: 'Go to the Kenya Power self-service portal (kplcustomer.com), log in with test credentials, navigate to Bill Payment, enter a test meter number, and screenshot the pre-filled bill details and the payment form. Do NOT complete payment.',
      taskType: 'agentic-ai', status: 'unclaimed', priority: 0.72,
      difficulty: 'easy', languageTags: ['en', 'sw'],
      externalUrl: 'https://www.kplc.co.ke/selfservice', estimatedDuration: 20,
    ...
    const t17 = await Task.create({
      batchId: batchWebGov._id, batchTitle: batchWebGov.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Request NHIF card replacement via the self-service portal',
      description: 'Visit the NHIF member self-service portal (selfservice.nhif.or.ke), log in with test credentials, navigate to Card Services → Request Replacement Card, fill in the reason and delivery address using placeholder data, and screenshot all steps through to the confirmation.',
      taskType: 'agentic-ai', status: 'submitted', priority: 0.70,
      difficulty: 'medium', languageTags: ['en'],
      externalUrl: 'https://selfservice.nhif.or.ke', estimatedDuration: 25,
      annotatorId: njeri._id, annotatorEmail: njeri.email,
    ...
    const t18 = await Task.create({
      ...
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Order food on Glovo Kenya and reach the checkout confirmation',
      description: 'Go to Glovo Kenya (glovoapp.com/ke), navigate to a fast food restaurant (KFC or Chicken Inn), add at least two items to your cart, proceed to checkout with test address: Westgate Mall, Nairobi, and screenshot the order summary. Do NOT complete payment.',
      taskType: 'agentic-ai', status: 'unclaimed', priority: 0.78,
      difficulty: 'easy', languageTags: ['en'],
      externalUrl: 'https://glovoapp.com/ke', estimatedDuration: 22,
    ...
    const t19 = await Task.create({
      batchId: batchSocial._id, batchTitle: batchSocial.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Create a product listing for a used smartphone on Jiji Kenya',
      description: 'Go to Jiji.co.ke, navigate to Electronics → Phones, and create a new listing for a fictional used Samsung Galaxy A54 at KSh 25,000. Fill all required fields with test data and screenshot each step through to the listing preview page.',
      taskType: 'agentic-ai', status: 'unclaimed', priority: 0.75,
      difficulty: 'medium', languageTags: ['en'],
      externalUrl: 'https://jiji.co.ke', estimatedDuration: 30,
    ...
    const t20 = await Task.create({
      batchId: batchSocial._id, batchTitle: batchSocial.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Track a courier parcel on DHL Kenya portal',
      description: 'Go to the DHL Kenya tracking portal, enter test tracking number 1234567890, view the full shipment history timeline, and screenshot each step of the tracking flow including the final delivery status page.',
      taskType: 'agentic-ai', status: 'in-progress', priority: 0.72,
      difficulty: 'easy', languageTags: ['en'],
      externalUrl: 'https://www.dhl.com/ke-en/home/tracking.html', estimatedDuration: 15,
      annotatorId: kipchoge._id, annotatorEmail: kipchoge.email,
      startedAt: ago(h(0.5)),
    ...
    const t21 = await Task.create({
      batchId: batchSocial._id, batchTitle: batchSocial.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Search and save property listings on BuyRentKenya',
      description: 'Go to BuyRentKenya.com, search for 2-bedroom apartments for rent in Westlands, Nairobi (KSh 30,000–50,000/month), open the top three results, and save each one. Screenshot search results, each property detail page, and the saved listings page.',
      taskType: 'agentic-ai', status: 'submitted', priority: 0.70,
      difficulty: 'medium', languageTags: ['en'],
      externalUrl: 'https://www.buyrentkenya.com', estimatedDuration: 28,
      annotatorId: wanjiru._id, annotatorEmail: wanjiru.email,
    ...
    const t22 = await Task.create({
      batchId: batchSocial._id, batchTitle: batchSocial.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Book a Bolt ride via the Bolt web booking portal',
      description: 'Go to Bolt web booking (bolt.eu/ke), enter pickup: Westgate Mall, Nairobi and drop-off: JKIA Airport, select the Bolt Go category, review the fare estimate, and screenshot all steps through to the booking confirmation. Do NOT confirm the ride.',
      taskType: 'agentic-ai', status: 'data-ready', isLocked: true, priority: 0.74,
      difficulty: 'easy', languageTags: ['en'],
      externalUrl: 'https://bolt.eu/ke', estimatedDuration: 18,
      annotatorId: njeri._id, annotatorEmail: njeri.email,
      reviewerId: mutua._id, reviewerEmail: mutua.email,
      qualityScore: 90,
    ...
    const t23 = await Task.create({
      batchId: batchSocial._id, batchTitle: batchSocial.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Search and book a hotel room on Jumia Travel Kenya',
      description: 'Go to Jumia Travel Kenya, search for hotels in Nairobi CBD for next weekend (1 night, 1 adult), open the top 2 results, compare amenities and prices, and add one to your booking cart. Screenshot search results, each hotel detail page, and the booking cart.',
      taskType: 'agentic-ai', status: 'unclaimed', priority: 0.68,
      difficulty: 'easy', languageTags: ['en'],
      externalUrl: 'https://travel.jumia.com/kenya', estimatedDuration: 25,
    ...
    const t24 = await Task.create({
      batchId: batchSocial._id, batchTitle: batchSocial.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Register as a service provider on Lynk Kenya',
      description: 'Go to Lynk.ke, navigate to the service provider registration flow, fill in a test profile as a plumber, upload a placeholder portfolio image, and complete the registration form. Screenshot every step through to the profile preview.',
      taskType: 'agentic-ai', status: 'submitted', priority: 0.66,
      difficulty: 'medium', languageTags: ['en'],
      externalUrl: 'https://lynk.ke', estimatedDuration: 30,
      annotatorId: odhiambo._id, annotatorEmail: odhiambo.email,
    ...
    const t25 = await Task.create({
      ...
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Log in and check account balance on KCB Internet Banking',
      description: 'Go to KCB Internet Banking (ibank.kcbgroup.com), log in with test credentials, navigate to Account Summary, and screenshot the full balance overview page. Do NOT initiate any transactions.',
      taskType: 'agentic-ai', status: 'unclaimed', priority: 0.82,
      difficulty: 'easy', languageTags: ['en'],
      externalUrl: 'https://ibank.kcbgroup.com', estimatedDuration: 15,
    ...
    const t26 = await Task.create({
      batchId: batchFintech._id, batchTitle: batchFintech.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Apply for an NCBA Loop digital loan via the web portal',
      description: 'Go to the NCBA Loop website, navigate to the loan application section, and fill in the application form using test data (loan amount: KSh 50,000). Screenshot each step of the multi-page form through to the loan summary page.',
      taskType: 'agentic-ai', status: 'unclaimed', priority: 0.79,
      difficulty: 'hard', languageTags: ['en'],
      externalUrl: 'https://www.ncbaloop.com', estimatedDuration: 40,
    ...
    const t27 = await Task.create({
      batchId: batchFintech._id, batchTitle: batchFintech.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Initiate a Pesalink transfer on Co-operative Bank portal',
      description: 'Log into the Co-operative Bank online portal with test credentials, navigate to Fund Transfers → Pesalink, fill in a test transfer (KSh 5,000 to a fictitious recipient), and screenshot each step through to the transfer confirmation. Do NOT submit the transfer.',
      taskType: 'agentic-ai', status: 'in-progress', priority: 0.77,
      difficulty: 'hard', languageTags: ['en'],
      externalUrl: 'https://www.co-opbank.co.ke/online-banking', estimatedDuration: 35,
      annotatorId: odhiambo._id, annotatorEmail: odhiambo.email,
      startedAt: ago(h(1)),
    ...
    const t28 = await Task.create({
      batchId: batchFintech._id, batchTitle: batchFintech.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Download a 3-month bank statement on Equity Bank online',
      description: 'Log into Equity Bank online banking with test credentials, navigate to Statements, select the last 3 months date range, generate the PDF statement, and screenshot the statement PDF preview. Note the exact navigation path used.',
      taskType: 'agentic-ai', status: 'submitted', priority: 0.75,
      difficulty: 'medium', languageTags: ['en'],
      externalUrl: 'https://equitybankkenya.com/online-banking', estimatedDuration: 25,
      annotatorId: kipchoge._id, annotatorEmail: kipchoge.email,
    ...
    const t29 = await Task.create({
      batchId: batchFintech._id, batchTitle: batchFintech.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Send money internationally via M-Pesa GlobalPay',
      description: 'Go to the M-Pesa GlobalPay portal, navigate to Send Money → International Transfer, select United Kingdom as the destination, fill in test recipient details (name, bank account number), and screenshot each step through to the fee summary page. Do NOT confirm the transfer.',
      taskType: 'agentic-ai', status: 'unclaimed', priority: 0.73,
      difficulty: 'hard', languageTags: ['en'],
      externalUrl: 'https://www.safaricom.co.ke/personal/m-pesa/globalpay', estimatedDuration: 35,
    ...
    const t30 = await Task.create({
      batchId: batchFintech._id, batchTitle: batchFintech.title,
      workflowId: agenticWF._id, workflowName: 'Agentic AI Evaluation',
      title: 'Check and repay an M-Shwari loan balance online',
      description: 'Log into the M-Shwari portal via the Safaricom self-care website, navigate to Loans → Active Loans, view the outstanding balance and repayment schedule, and screenshot the loan details page and repayment form. Do NOT submit repayment.',
      taskType: 'agentic-ai', status: 'in-progress', priority: 0.71,
      difficulty: 'medium', languageTags: ['en', 'sw'],
      externalUrl: 'https://www.safaricom.co.ke/personal/m-pesa/m-shwari', estimatedDuration: 22,
      annotatorId: wanjiru._id, annotatorEmail: wanjiru.email,
      startedAt: ago(h(0.5)),
